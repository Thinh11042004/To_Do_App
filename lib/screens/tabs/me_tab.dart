...
class _MeTabState extends State<MeTab> {
  ...
  Widget build(BuildContext context) {
    ...
    final pendingTotal = pendingSlices.fold<int>(0, (sum, s) => sum + s.count);
      ...
      child: ListView(
        ...
        children: [
          ...
          Padding(
            padding: const EdgeInsets.fromLTRB(16, 16, 16, 8),
            child: Text(
              'Nhiệm vụ trong ${_days ?? 'tất cả'} ngày tới',
  ...
  List<Widget> _buildUpcomingList(BuildContext context, List<Task> filtered) {
    ...
    if (upcoming.isEmpty) {
      return const [
        SizedBox(
          height: 60,
          child: Center(child: Text('— Không có nhiệm vụ —')),
        ),
      ];
    }
  ...
  List<_DailyCompletionStat> _buildDailyCompletionStats(List<Task> tasks) {
    ...
    return List.generate(range, (index) {
      ...
      return _DailyCompletionStat(day: day, completed: completed, total: total);
    });
  }

  List<_CategorySlice> _pendingSlices(List<Task> tasks, ColorScheme scheme) {
    ...
    if (others > 0) {
      slices.add(
        _CategorySlice(label: 'Khác', count: others, color: scheme.tertiary),
      );
    }
    return slices;
  }
...
class _DailyCompletionChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final maxTotal = visible.fold<int>(0, (prev, s) => math.max(prev, s.total));
    if (maxTotal == 0) {
      return SizedBox(
        height: maxBarHeight,
        child: Center(
                        ...
                        children: [
                          ...
                          if (stat.total > 0)
                            Align(
                              ...
                              child: Container(
                                width: barWidth,
                                height: maxBarHeight * (stat.total / maxTotal),
                                decoration: BoxDecoration(
                          ...
                          if (stat.completed > 0)
                            Align(
                              ...
                              child: Container(
                                width: barWidth,
                                height:
                                    maxBarHeight * (stat.completed / maxTotal),
                                decoration: BoxDecoration(
                                  gradient: LinearGradient(

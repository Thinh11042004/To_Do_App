...
final TaskRepository _repo = DbService.tasks;
  ...
  Task _fromEntity(TaskEntity e) {
    ...
    return Task(
      id: (e.id ?? 0).toString(),
      title: e.title,
      category: _categoryFromId(e.categoryId), // TODO: map từ e.categoryId nếu bạn dùng
      dueDate: due,
      timeOfDay: timeOfDay, // TODO: nếu cần, tách giờ phút từ due
  ...
  TaskEntity _toEntity(Task t) {
    ...
    return TaskEntity(
      ...
      priority: 'normal',
       categoryId: _categoryToId(t.category), // TODO: map từ t.category nếu bạn dùng
      tags: const [],
      createdAt: now,
      updatedAt: now,
    );
  ...
  Future<void> _addTask(Task newTask) async {
    ...
    final id = await _repo.add(e);
    await NotificationService.instance.scheduleForTask(e.copyWith(id: id));
  }

  Future<void> _updateTask(Task t) async {
    final e = _toEntity(t).copyWith(id: int.tryParse(t.id));
    if (e.id == null) return;
    await _repo.update(e.copyWith(updatedAt: DateTime.now()));
    if (t.done) {
      await NotificationService.instance.cancelReminder(e.id);
    } else {
      await NotificationService.instance.scheduleForTask(e);
    }
  }
